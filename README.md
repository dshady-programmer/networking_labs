...
├── lab-05-northline/
├── lab-06-vantage/
└── lab-07-coastline-ospf/      (bonus: OSPF rebuild once I've learned it)
```
...
| # | Lab | Focus | Status |
|---|-----|-------|--------|
| 1 | [Bright Leaf Accounting](#lab-01--bright-leaf-accounting-small-office) | VLANs, DHCP, inter-VLAN routing | ⬜ Not started |
| 2 | [Coastline Retail — Static](#lab-02--coastline-retail-group-2-site-wan-static-routing) | Static routing, ACLs, multi-site addressing | ⬜ Not started |
| 3 | [Coastline Retail — EIGRP rebuild](#lab-03--coastline-retail-group-rebuild-with-eigrp) | EIGRP basics | ⬜ Not started |
| 4 | [Summit Ridge College](#lab-04--summit-ridge-college-campus-network) | Redundant switching, STP | ⬜ Not started |
| 5 | [Northline Logistics](#lab-05--northline-logistics-nat-security-wireless) | NAT/PAT, wireless, port security | ⬜ Not started |
| 6 | [Vantage Financial Group](#lab-06--vantage-financial-group-enterprise-capstone) | OSPF at scale, FHRP, summarization | ⬜ Not started |
| 7 (bonus) | [Coastline Retail — OSPF rebuild](#lab-07-bonus--coastline-retail-group-rebuild-with-ospf) | OSPF basics, EIGRP vs. OSPF comparison | ⬜ Not started |
...
**The brief:** Bright Leaf Accounting is a 3-partner firm moving into a new single-floor office. Three departments — Accounting (12 staff), Admin/Reception (4 staff), Management (3 staff) — each need to be logically separated for security, but everyone needs internet access and needs to share one network printer.
...
**What I'm practicing:** VLANs, trunking, inter-VLAN routing, DHCP, basic switchport security.

**Constraints:** Single floor, single switch stack is fine, one router.

➡️ Solution: `lab-01-bright-leaf/` *(coming soon)*
...
**What I'm practicing:** Static routing, standard/extended ACLs, multi-site addressing (no overlapping subnets).

**Constraint:** No dynamic routing yet — management "doesn't trust that automatic stuff." Static routes only, for now.

➡️ Solution: `lab-02-coastline-static/` *(coming soon)*
...
**What I'm practicing:** EIGRP basics (neighbor relationships, the composite bandwidth/delay metric, convergence), comparing static vs. dynamic routing tradeoffs on the same topology.

**Note:** Going with EIGRP first since it's Cisco-proprietary but simpler to get running than OSPF (no areas or network-type mismatches to fight with). I'll circle back with an OSPF rebuild once I've learned it — see Lab 07.

➡️ Solution: `lab-03-coastline-eigrp/` *(coming soon)*

---

## Lab 04 — Summit Ridge College (Campus Network)
...
➡️ Solution: `lab-04-summit-ridge/` *(coming soon)*

---

## Lab 05 — Northline Logistics (NAT, Security, Wireless)

**The brief:** Northline runs a warehouse + office combo. Office staff use wired PCs; warehouse floor staff use handheld scanners over Wi-Fi. They have one public IP from their ISP. A former employee's old switch port once let a guest laptop onto the internal network from the lobby — security is now a big concern.
...
➡️ Solution: `lab-05-northline/` *(coming soon)*

---

## Lab 06 — Vantage Financial Group (Enterprise, Capstone)
...
**What I'm practicing:** OSPF at scale, FHRP (HSRP/VRRP), route summarization, ACLs at scale, professional addressing design.

**Constraint:** At least 4 sites total (HQ + 3 branches). This is the capstone — it pulls together everything from Weeks 1–5.

➡️ Solution: `lab-06-vantage/` *(coming soon)*

---

## Lab 07 (bonus) — Coastline Retail Group, Rebuild with OSPF

**The brief:** Same company, same topology as Labs 02–03 — one more pass, this time with OSPF, once I've actually learned it properly.
...
➡️ Solution: `lab-07-coastline-ospf/` *(coming soon)*
